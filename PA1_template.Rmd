...
-->
```{r extraDayStudy, results="hide", echo=FALSE, fig.keep="none"}
temp <- newActivity %>% ungroup() %>%
    mutate(day = as.factor(weekdays(newActivity$date))) %>%
    group_by(interval, day) %>%
    summarise_each(funs(mean(., na.rm = TRUE)), steps) %>%
    arrange(interval, day)

xTickAt <- seq(1, 12*24, by=12) # one label every hour on the x axis
xLabels <- temp$interval[xTickAt * 7]
q <- xyplot(steps ~ interval | levels(day), data=temp, type="l",
            layout=c(2,4), main="Average # steps per 5 minute interval",
            xlab="5-minute Interval", ylab="Average Number of steps",
            scales=list(x=list(at=xTickAt, labels=xLabels, rot=90)))
print(q)
```
...
rm(p, q, xLabels, xTickAt, naValues, maxEntry, transBlue, transRed, meanDiff,
   medianDiff, dayTypeColumn, zeroDays, totalDays, newZeroDays, reduction, temp)
```
